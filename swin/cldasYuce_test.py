...
index=np.load("/home/hyc/swin/swin/index.npy")
...
input = layers.Input(shape=(image_dimension[0]//2//2*image_dimension[1]//2//2, 2*2*2*7))
...
# x=layers.Flatten()(x)
output = layers.Dense((2*2*4*4*4*4), activation="relu")(x)
# output = layers.Dense((image_dimension[0]*image_dimension[1]* 2*2), activation="relu")(x)
output=layers.Reshape((image_dimension[0], image_dimension[1], 2,2))(output)
...
jilu_pre_tmp=np.load('/home/hyc/swin/swin/jilu_pre_tmp.npy')
import os
...
model.load_weights("/home/hyc/swin/swin/my_model_save_weights_cldas.weights.h5")
result=model.predict(dataset_test,batch_size=data_batch_size)
...
print("rmse",rmse(result[:,:,:,0,:],y_test_all[:,:,:,0,:])*(jilu_pre_tmp[0][0]-jilu_pre_tmp[0][1]))
def mae(y1,y2):
    y1=y1.flatten()
    y2=y2.flatten()
    return np.sum(np.abs(y1-y2))/len(y1)
print("mae",mae(result[:,:,:,0,:],y_test_all[:,:,:,0,:])*(jilu_pre_tmp[0][0]-jilu_pre_tmp[0][1]))
def r2(y1,y2):
    y1=y1.flatten()
    y2=y2.flatten()
...
print("tmp")
# print("aucc",aucc)
print("rmse",rmse(result[:,:,:,1,:],y_test_all[:,:,:,1,:])*(jilu_pre_tmp[0][0]-jilu_pre_tmp[0][1]))
print("mae",mae(result[:,:,:,1,:],y_test_all[:,:,:,1,:])*(jilu_pre_tmp[0][0]-jilu_pre_tmp[0][1]))
print("r2",r2(result[:,:,:,1,:],y_test_all[:,:,:,1,:]))
