...
def build_model(hp):
    embed_dim=hp.Choice(name="embed_dim", values=[64,128,256]) 
    num_heads=hp.Choice(name="num_heads", values=[4,8,16]) 
    num_mlp=hp.Choice(name="num_mlp", values=[128,256,512]) 
    # qkv_bias=hp.Choice(name="qkv_bias", values=[True, False]) 
    dropout_rate=hp.Choice(name="dropout_rate", values=[0.01, 0.05,0.1]) 
    learning_rate=hp.Choice(name="learning_rate", values=[1e-2, 1e-3,1e-4]) 
    weight_decay=hp.Choice(name="weight_decay", values=[0.00001, 0.0001,0.001]) 
    input = layers.Input(shape=(32*32, 2*2*27))
    ...
    output = layers.Dense((24*4*4), activation="relu")(x)
...
tuner = Hyperband(
    ...
    objective='val_loss',
    max_epochs=150,
    directory='/home/hyc/swin/swin',
    project_name='chaocan_opt1')
tuner.search(dataset,validation_data=dataset_val)
...
print("rmse",rmse(result,y_test))
def mae(y1,y2):
    y1=y1.flatten()
    y2=y2.flatten()
    return np.sum(np.abs(y1-y2))/len(y1)
print("mae",mae(result,y_test))
def r2(y1,y2):
    y1=y1.flatten()
    y2=y2.flatten()
