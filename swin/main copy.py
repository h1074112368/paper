...
embed_dim = 64*2*2  # Embedding dimension
...
num_patch_x = input_shape[0] // patch_size[0]*24
...
for i in range(0,3000):
    ...
    ncDataset=nc.Dataset(f"/home/hyc/new_make_train/flood_train_set_new/wdept{'%04d'% i}.nc")
...
x_train, x_test = x[index_train].reshape((-1,24*64,64,4)), x[index_test].reshape((-1,24*64,64,4))
...
def patch_extract(images):
    ...
    patch_num = int(patches.shape[1]/24)
    return tf.reshape(patches, (batch_size, 24*patch_num * patch_num, patch_dim))
...
def augment(x):
    x = tf.image.random_crop(x, size=(24*image_dimension[0], image_dimension[1], image_dimension[2]))#3))
...
input = layers.Input(shape=(1024*24, 4*4))
...
output=layers.Reshape((24,64,64))(output)
...
import os
if(os.path.exists("/home/hyc/swin/swin/my_model.keras")):
    os.unlink("/home/hyc/swin/swin/my_model.keras")
keras.saving.save_model(model,"/home/hyc/swin/swin/my_model.keras")
model.save_weights("/home/hyc/swin/swin/my_model_save_weights.weights.h5")
