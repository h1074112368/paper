...
def tiff2nc(path,chan,chandem,length):
    data = gdal.Open(path)
    ...
    def tianwa(demgrid,chan):
        import copy
        out=np.zeros([demgrid.shape[0]-2,demgrid.shape[1]-2],dtype='float32')
        for i in range(out.shape[0]):
            for j in range(out.shape[1]):
                if chan[i,j]==1:
                    tmp=copy.deepcopy(demgrid[i:i+3,j:j+3])#.copy(deep=True)
                    tmpn=tmp[1,1]
                    tmp[1,1]=9999
                    out[i,j]=np.min(tmp)
                    if((tmpn!=9999)&(out[i,j]==9999)):
                        out[i, j]=tmpn
                # }
        return out
    inp=tianwa(demgrid,chan)
    #
    # def slopeCal(dem,chan,ldd,length):
    #     out=np.zeros_like(chan,dtype='float32')
    #     for i in range(chan.shape[0]):
    #         for j in range(chan.shape[1]):
    #             if chan[i,j]==1:
    #                 if ldd[i,j]==1 or ldd[i,j]==9:
    #                     a1 = dem[i + 1 - 1, j + 1 - 1] - dem[i + 1, j + 1]
    #                     a2 = dem[i + 1 + 1, j + 1 + 1] - dem[i + 1, j + 1]
    #                     # a1=dem[i + 1 + 1, j + 1 - 1]-dem[i+1,j+1]
    #                     # a2=dem[i+1-1,j+1+1]-dem[i+1,j+1]
    #                     mint=np.max([a1,a2,0])
    #                     if mint==0:
    #                         out[i,j]=0.0001
    #                     else:
    #                         out[i,j]=length[i,j]/mint
    #
    #                 if ldd[i,j]==7 or ldd[i,j]==3:
    #                     a1=dem[i + 1 + 1, j + 1 - 1]-dem[i+1,j+1]
    #                     a2=dem[i+1-1,j+1+1]-dem[i+1,j+1]
    #                     # a1 = dem[i+1-1,j+1-1] - dem[i + 1, j + 1]
    #                     # a2 = dem[i+1+1,j+1+1] - dem[i + 1, j + 1]
    ...
    #                     else:
    #                         out[i, j] = length[i, j]/ mint
    #                 if ldd[i, j] == 2 or ldd[i, j] == 8:
    #                     a1 = dem[i + 1, j + 1 - 1] - dem[i + 1, j + 1]
    #                     a2 = dem[i + 1 , j + 1 + 1] - dem[i + 1, j + 1]
    #                     mint = np.max([a1,a2,0])
    #                     if mint == 0:
    #                         out[i, j] = 0.0001
    #                     else:
    #                         out[i, j] = length[i, j] / mint
    #
    #                 if ldd[i, j] == 4 or ldd[i, j] == 6:
    #                     a1 = dem[i + 1-1, j + 1 ] - dem[i + 1, j + 1]
    #                     a2 = dem[i + 1+1 , j + 1 ] - dem[i + 1, j + 1]
    #                     mint = np.max([a1,a2,0])
    #                     if mint == 0:
    #                         out[i, j] = 0.0001
    #                     else:
    ...
    ds = xr.Dataset({
        ...
        # coords={
        #     'value': (['lat', 'lon'], inp, t_coords)
        # }
       )
    # _FillValue
    # ds['value'].encoding('_FillValue') = -9999
    # ds['value'].__setattr__('_FillValue', -9999)
    # ds['value'].fillna(-9999)
    return ds

import  netCDF4 as nc
path=r'D:\tensorflow\lisflood\shanghai\maps'
chan=np.array(nc.Dataset(path+'/chan.nc')['value'])
chandem=np.array(nc.Dataset(path+'/chandem.nc')['value'])
length=np.array(nc.Dataset(path+'/chanleng.nc')['value'])
day_nc = tiff2nc(r"D:\tensorflow\lisflood\data_25m\dem_shanghai_25m_mask_re.tif",chan,chandem,length)

day_nc.to_netcdf(path+'/chandem_tianwa.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
