...
def tiff2nc(path,chanbw):
    ...
    for i in range(3):
        for j in range(3):
            ...
            inp[i,j]=z
    ds = xr.Dataset({
        'lon': (['lon'], im_lon[:3], lon_attr),
        'lat': (['lat'], im_lat[:3], lat_attr),

        'value': (['lat', 'lon'], inp, t_attr)},
    )
    return ds

import  netCDF4 as nc
path=r'D:\tensorflow\lisflood\shanghai\maps'
chanbw=np.array(nc.Dataset(path+'/chan.nc')['value'])
day_nc = tiff2nc(r"D:\tensorflow\3dTest\dem_re_clip.tif",chanbw)
