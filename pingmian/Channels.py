...
def tiff2nc(path,mask):
    data = gdal.Open(path)
    # data_w=gdal.Open(path_w)
    ...
    # im_data_w=data_w.GetRasterBand(1).ReadAsArray()
    # 根据im_proj得到图像的经纬度信息
    ...
    # inp=np.where(((im_data==60) & (mask==1)),1,0)
    inp=np.ones_like(im_data)
    # inp[np.where(im_data==60)]=1
    ds = xr.Dataset({
         'x': (['x'], im_lon, lon_attr),
        'y': (['y'], im_lat, lat_attr),
...
# path_dem=r"D:\tensorflow\3dTest\landuse_re_clip.tif"
path=r'D:\tensorflow\lisflood\shanghai\maps'
# path_w=r"D:\tensorflow\lisflood\water_re.tif"
# gdal.Warp(path_w,path_w)
mask=np.array(nc.Dataset(path+'/area.nc')['value'])
day_nc = tiff2nc(r"D:\tensorflow\lisflood\data_25m\landUseAddWudingAddwater.tif",mask)
path=r'D:\tensorflow\lisflood\shanghai\maps'
day_nc.to_netcdf(path+'/chan.nc')
