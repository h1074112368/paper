...
def tiff2nc(path):
    ...
    inp=im_data#np.ones([im_data.shape[0],im_data.shape[1]],dtype='float32')*0.25
    inp[inp > 0.42] = 0.42
...
day_nc = tiff2nc(r"G:\data_FLOOD\soil\cal\Lamba_layer2.tif")
path=r'D:\tensorflow\lisflood\shanghai\maps'
day_nc.to_netcdf(path+'/lambda2_o.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
