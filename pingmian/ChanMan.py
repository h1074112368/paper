...
def tiff2nc(path,chan,uA,area):
    ...
    #                 a=0
    #                 jinji1=set()
    #                 while chan[x,y]==1 and jinji[x,y]==0:
    #                     if (x,y) in set:
    #                         break
    #                     set.add((x,y))
    #                     if chan[x,y] == 1:
    ...
    #
    #     return out
    # upA=upsteamArea(dem,chan,ldd,area)
    inp=np.zeros_like(uA,dtype='float32')
    for i in range(uA.shape[0]):
        for j in range(uA.shape[1]):
            inp[i,j]=0.023+0.015*min(50/(uA[i,j]+0.00000001),1)+0.30*min(im_data[i,j]/2000,1)
    # inp=0.023+0.015*np.min(50/(uA+0.00000001),1)+0.30*min()
...
day_nc = tiff2nc(r"D:\tensorflow\lisflood\dem_shanghai_25m\dem_shanghai_25m.tif",chan,uA,area)

day_nc.to_netcdf(path+'/chanman.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
