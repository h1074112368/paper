...
def tiff2nc(path):
    ...
    def flow_f(data):
        ...
        demcha=np.zeros([data.shape[0] - 2, data.shape[1] - 2], dtype=int)
            ...
            for j in range(flow.shape[1]):
                ...
                if mint == 0:
                    ...
                    if minidx==1:
                        ...
                        flow[i, j] = 8
                        demcha[i, j+1] = temp[i, j+1]
                    elif minidx==3:
                        flow[i, j] = 9
                        demcha[i, j+2] = temp[i, j+2]
                    elif minidx==7:
                        flow[i, j] = 1
                        demcha[i+2, j] = temp[i+2, j]
                    elif minidx==8:
                        flow[i, j] = 2
                        demcha[i+2, j+1] = temp[i+2, j+1]
                    elif minidx==9:
                        flow[i, j] = 3
                        demcha[i+2, j+1] = temp[i+2, j+1]
                    elif minidx == 5:
                        flow[i, j] = 5
                        demcha[i+1, j+1] = temp[i+1, j+1]
                    elif minidx == 4:
                        flow[i, j] = 4
                        demcha[i+1, j] = temp[i+1, j]
                    elif minidx == 6:
                        flow[i, j] = 6
                        demcha[i+1, j+2] = temp[i+1, j+2]
                    else:
                        flow[i, j]=minidx
        # for i in range(flow.shape[0]):
        #     for j in range(flow.shape[1]):
