...
def tiff2nc(path):
    ...
    # laea_attr=dict()
    time_data=pd.date_range('2015-01-01 00:00:00','2015-12-20 00:00:00',freq="10D")
    ...
    for i in range(len(time_data)):
        inp[i]+=im_data
        inp[i]*=0.1
        # tmp= ((np.random.normal(size=[1,im_data.shape[0], im_data.shape[1]]) + 1) / 2)
        # inp.append( tmp.astype('float32'))
    # inp=np.concatenate(inp,axis=0)
    # inp=np.ones([time_data.shape[0],im_data.shape[0], im_data.shape[1]],dtype='float32')*0
...
day_nc = tiff2nc(r"F:\LAI\re_GLASS01D01.V60.A2021209.h28v05.2022151.tif")
path=r'D:\tensorflow\lisflood\shanghai\lai'
day_nc.to_netcdf(path+'/LAIMaps.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
