...
def tiff2nc(path):
    ...
    # def gra(data):
    inp=np.zeros_like(im_data,dtype=int)
    inp[int((3635888.683-im_geotrans[3])/im_geotrans[5]),int((13479395.636-im_geotrans[0])/im_geotrans[1])]=1
    inp[int((3627514.437-im_geotrans[3])/im_geotrans[5]),int((13487423.449-im_geotrans[0])/im_geotrans[1])]=2
    inp[int((3635369.138 - im_geotrans[3]) / im_geotrans[5]), int((13523701.6 - im_geotrans[0]) / im_geotrans[1])] = 3
    inp[int((3702699.487 - im_geotrans[3]) / im_geotrans[5]), int((13516695.504 - im_geotrans[0]) / im_geotrans[1])] = 4
    # inp[3000,2000]=1
    # inp[2000,3000]=2
    # for i in range(len(time_data)):
    #     tmp= ((np.random.normal(size=[im_data.shape[0], im_data.shape[1]]) + 1) / 2)
    #     inp.append( tmp.astype('float32'))
    # inp=np.concatenate(inp,axis=0)
    # inp=(np.random.normal(size=[im_data.shape[0],im_data.shape[1]],dtype='float32')+1)/2
    ds = xr.Dataset({
        'x': (['x'], im_lon, lon_attr),
        'y': (['y'], im_lat, lat_attr),
...
day_nc = tiff2nc(r"D:\tensorflow\lisflood\data_25m\demAddWuding.tif")
path=r'D:\tensorflow\lisflood\shanghai\inflow'
day_nc.to_netcdf(path+'/outlets.nc')
