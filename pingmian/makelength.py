...
def tiff2nc(path):
    ...
    inp=np.ones([im_data.shape[0],im_data.shape[1]],dtype='float32')*25#*6371*1000*np.arccos(np.cos(im_lat[0]/180*np.pi)*np.cos(im_lat[0]/180*np.pi)*np.cos(im_lon[0]/180*np.pi-im_lon[1]/180*np.pi)+np.sin(im_lat[0]/180*np.pi)*np.sin(im_lat[0]/180*np.pi))
...
day_nc.to_netcdf(path+'/length.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
