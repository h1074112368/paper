...
def tiff2nc(path,length,chan):
    ...
    inp=np.ones([im_data.shape[0],im_data.shape[1]],dtype='float32')*np.sqrt(2)*length[0]
...
day_nc = tiff2nc(r"D:\tensorflow\lisflood\dem_shanghai_25m\dem_shanghai_25m.tif",length,chan)
path=r'D:\tensorflow\lisflood\shanghai\maps'

day_nc.to_netcdf(path+'/chanleng.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
