...
def tiff2nc(path):
    ...
    inp = im_data#np.ones([im_data.shape[0], im_data.shape[1]], dtype='float32') * 1*0.02
    inp[inp > 0.055] = 0.055
    ds = xr.Dataset({
         'x': (['x'], im_lon, lon_attr),
        'y': (['y'], im_lat, lat_attr),
...
day_nc = tiff2nc(r"G:\data_FLOOD\soil\cal\Alpha_layer2.tif")
path=r'D:\tensorflow\lisflood\shanghai\maps'
day_nc.to_netcdf(path+'/genua2_o.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
