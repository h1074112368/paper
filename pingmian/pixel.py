...
def tiff2nc(path,length):
    ...
    inp=np.ones([im_data.shape[0],im_data.shape[1]],dtype='float32')*length[0,0]*length[0,0]
...
day_nc = tiff2nc(r"D:\tensorflow\lisflood\data_25m\landUseAddWudingAddwater.tif",length)

path=r'D:\tensorflow\lisflood\shanghai\maps'
day_nc.to_netcdf(path+'/pixel.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
