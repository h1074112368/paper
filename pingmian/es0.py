...
def tiff2nc(path):
    ...
    # laea_attr=dict()
    time_data=pd.date_range('2021-07-23 00:00:00','2021-07-26 23:00:00',freq="H")
    ...
    inp = np.ones([time_data.shape[0], im_data.shape[0], im_data.shape[1]], dtype='float32') * 0.0
    ds = xr.Dataset({
        'x': (['x'], im_lon, lon_attr),
        'y': (['y'], im_lat, lat_attr),
...
day_nc = tiff2nc(r"D:\tensorflow\lisflood\data_25m\landUseAddWudingAddwater.tif")
path=r'D:\tensorflow\lisflood\shanghai\meteo'
day_nc.to_netcdf(path+'/es0.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
