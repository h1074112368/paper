...
chanbw=np.array(nc.Dataset(path+'/chanbw.nc')['value'])
chan=np.array(nc.Dataset(path+'/chan.nc')['value'])
day_nc = tiff2nc(r"D:\tensorflow\lisflood\dem_shanghai_25m\dem_shanghai_25m.tif",chanbw,chan)

day_nc.to_netcdf(path+'/chanflpn.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
