...
def tiff2nc(path):
    ...
    inp=im_data*10#np.ones([im_data.shape[0],im_data.shape[1]],dtype='float32')*50
    inp[inp < 0] = -9999
...
day_nc = tiff2nc(r"G:\data_FLOOD\soil\cal\thickness1.tif")
path=r'D:\tensorflow\lisflood\shanghai\soilhyd'
day_nc.to_netcdf(path+'/soildepth1_o.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
