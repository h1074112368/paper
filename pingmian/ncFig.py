...
ncSet=np.array(nc.Dataset(r"D:\tensorflow\lisflood\yanhua_wdept.nc")['wdept'])
...
# ncSet*=100
max=np.max(ncSet)
ncSet[np.where((ncSet<150)&(ncSet>0))]=1
...
# ncSet[np.where((ncSet<400)&(ncSet>=270))]=3
# ncSet[np.where((ncSet<270)&(ncSet>=150))]=2
# ncSet[(ncSet<=14)&(ncSet>0)]=1
# ncSet[np.where((ncSet<150)&(ncSet>0))]=1
ncSet=ncSet.astype('int')
# ncSet[ncSet>70]=1
# ncSet[(ncSet<=70)&(ncSet>20)]=2
# ncSet[ncSet<=20]=3
# ncSet[(ncSet<=70)&(ncSet>20)]
# min=np.min(data)
# data[data==-9999]=0

import matplotlib.pyplot as plt
for i in range(24,ncSet.shape[0]):
     # import matplotlib
