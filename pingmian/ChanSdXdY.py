...
def tiff2nc(path,chan,ldd,length):
    data = gdal.Open(path)
    ...
    inp=slopeCal(demgrid, chan, ldd,length)
...
ldd=np.array(nc.Dataset(path+'/ldd_nottw_CNBH.nc')['value'])
length=np.array(nc.Dataset(path+'/chanleng.nc')['value'])
day_nc = tiff2nc(r"D:\tensorflow\lisflood\dem_shanghai_25m\dem_shanghai_25m.tif",chan,ldd,length)

day_nc.to_netcdf(path+'/chans.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
