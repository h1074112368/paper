...
# data=gdal.Open(r"D:\tensorflow\lisflood\avgdis.map")
# tif=data.GetRasterBand(1).ReadAsArray()
from osgeo import gdal, ogr, gdalconst
import xarray as xr
import pandas as pd
import numpy as np
...
def tiff2nc(path):
    ...
    # im_data[im_data==-340282346638528859811704183484516925440.00000]=0.0000000000001
    # im_data*=100
    im_data[np.isnan(im_data)]=0.000000000000001
    # inp=np.ones([im_data.shape[0],im_data.shape[1]],dtype='float32')*6371*1000*np.arccos(np.cos(im_lat[0]/180*np.pi)*np.cos(im_lat[0]/180*np.pi)*np.cos(im_lon[0]/180*np.pi-im_lon[1]/180*np.pi)+np.sin(im_lat[0]/180*np.pi)*np.sin(im_lat[0]/180*np.pi))
    inp=im_data
    # for i in range(len(im_lon)):
    #     inp[:,i]=im_lon[i]
...
day_nc = tiff2nc(r"D:\tensorflow\lisflood\shanghai\out\avgdis.map")
path=r'D:\tensorflow\lisflood\shanghai\out'
day_nc.to_netcdf(path+'/avgdis.nc')
