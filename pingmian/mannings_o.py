...
def tiff2nc(path):
    ...
    inp=np.ones([im_data.shape[0],im_data.shape[1]],dtype='float32')*0.013
    inp[np.where(inp==30)]=0.24
    inp[np.where(inp == 10)] = 0.17
...
day_nc.to_netcdf(path+'/mannings_o.nc',encoding={
            'value':{
            '_FillValue':-9999}
        })
